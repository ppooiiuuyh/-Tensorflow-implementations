...
class Dataset_loader:
    def __init__(self,pvdir,trainset_ratio = 0.7,timestep_interval =2,duration_hour =3,duration_hour_long = 21,attList =[3], attList_long = [4,5]):
        self.timestep_interval = timestep_interval # 4: 1hour, 2: 30min, 1: 15min
        self.duration_hour = duration_hour
        self.duration_hour_long = duration_hour_long
        self.duration = self.duration_hour * 4/self.timestep_interval
        self.duration_long = self.duration_hour_long * 4/self.timestep_interval

        self.pv_origin = np.array(pd.read_csv(pvdir))[:,:].astype(float)
        self.attribute_list = attList
        self.attribute_list_long = attList_long

        self.num_attribute = len(self.attribute_list)#self.pv_origin.shape[1]
        self.num_attribute_long = len(self.attribute_list_long)#self.pv_origin.shape[1]
...
def module_test():
    dl = Dataset_loader(pvdir = "./data/pv_2016_gs_processed.csv")
    pass
