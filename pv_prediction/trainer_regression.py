...
from model_regression import Model

class Trainer:
    def __init__(self):
        #parameters
        self.totalEpoch = 20000
        self.batchSize = 128
        ...
        #model tensor :
        self.numClasses = 149
        self.model = Model(input_dim = self.dataset.duration*6+2,output_dim = self.numClasses)
    ...
    # ==========================================================
        ...
        with tf.name_scope("trainer") as scope:
            loss = tf.reduce_mean(tf.losses.mean_squared_error(labels= self.Y,predictions=self.model.logits_relu)) #+lossL2
    ...
    # ==================================================
        ...
        # ==============================================
            ...
            writer_acc_loss = tf.summary.FileWriter("./board_rw/acc_loss", sess.graph)
            ...
            writer_pred = tf.summary.FileWriter("./board_rw/pred", sess.graph)
            writer_pred_label = tf.summary.FileWriter("./board_rw/pred_label", sess.graph)
        ...
        #===============================================
            ...
            # .........................
                ...
                for i in range(int(len(self.trainset) / self.batchSize)):
                    ...
                    lr_value = 0.0001 * pow((1 - min(e / self.totalEpoch, 1)), 0.9)

                    sess.run(train_step, feed_dict={self.model.X: batch_x, self.Y: batch_y, self.model.trainphase: True,self.lr:lr_value})
            ...
            # 매epoch 마다 test 데이터셋에 대한 정확도와 loss를 출력.
                ...
                if(e%histloginterval == 0):
                    writer_pred = tf.summary.FileWriter("./board_rw/pred"+str(e), sess.graph)
                    writer_pred_label = tf.summary.FileWriter("./board_rw/pred_label"+str(e), sess.graph)
                ...
                for i in range(int(len(self.testset) / self.batchSize_test)):
                    ...
                    # == logging
                    test_accuracy,test_accuracy_square = sess.run([correct_prediction,correct_prediction_square],feed_dict={self.model.X: test_batch_x, self.Y: test_batch_y, self.model.trainphase: True})
                    test_accuracy_list.append(test_accuracy)
                    test_accuracy_s_list.append(test_accuracy_square)

                    if (e % histloginterval == 0):
                        for o in range(len(test_batch_y)):
                            output = self.model.logits_relu.eval(feed_dict={self.model.X: test_batch_x, self.Y: test_batch_y, self.model.trainphase: True})
                ...
                summary = merged.eval(
                    feed_dict={self.model.X: batch_x, self.Y: batch_y,
                               self.model.trainphase: True, accuracy_hist: np.mean(test_accuracy_list),
