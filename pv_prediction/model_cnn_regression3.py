...
class Model:
    def __init__(self,input_dim,output_dim,duration,modelname = ""):
        ...
        self.X = tf.placeholder(tf.float32,shape=[None,self.duration,6,1])
        self.SKY = tf.placeholder(tf.float32,shape=[None,1])
        self.RAIN = tf.placeholder(tf.float32,shape=[None,1])
    ...
    def apply_logit(self):
        ...
        with tf.variable_scope(self.modelname+"conv_2") as scope:
            self.W_conv2 = tf.get_variable(name='weights', shape=[2, 6, 1, 6],initializer=xavier_initializer_conv2d())
            ...
            self.h_conv2 = tf.nn.relu(self.h_bn2)

        with tf.variable_scope(self.modelname + "conv_3") as scope:
            self.W_conv3 = tf.get_variable(name='weights', shape=[3, 6, 1, 6], initializer=xavier_initializer_conv2d())
            ...
            self.h_conv3 = tf.nn.relu(self.h_bn3)


        self.pooled2 = tf.reshape(self.h_conv2,shape=[-1,self.duration-1,6])
        self.pooled2 = tf.reduce_mean(self.pooled2,reduction_indices=1)
        self.pooled2 = tf.squeeze(self.pooled2)



        self.pooled3 = tf.reshape(self.h_conv3,shape=[-1,self.duration-2,6])
        self.pooled3 = tf.reduce_mean(self.pooled3,reduction_indices=1)
        self.pooled3 = tf.squeeze(self.pooled3)




        self.pooled2_flatten = tf.reshape(self.pooled2,shape=[-1,6])
        self.pooled3_flatten = tf.reshape(self.pooled3,shape=[-1,6])

        self.flatten = tf.concat([self.pooled2_flatten,self.pooled3_flatten],1)
        self.flatten = tf.concat([self.flatten,self.SKY],1)
        self.flatten = tf.concat([self.flatten,self.RAIN],1)

        num_unit = 300
        H = self.linear(self.flatten,self.pooled2_flatten.shape[1]+self.pooled3_flatten.shape[1]+2,num_unit,'L1')
