...
class Model:
    ...
    def apply_logit(self):
        ...
        with tf.variable_scope(self.modelname+"conv_2") as scope:
            self.W_conv2 = tf.get_variable(name='weights', shape=[6, 2, 1, 6],initializer=xavier_initializer_conv2d())
        ...
        with tf.variable_scope(self.modelname + "conv_3") as scope:
            self.W_conv3 = tf.get_variable(name='weights', shape=[6, 3, 1, 6], initializer=xavier_initializer_conv2d())
        ...
        self.pooled2 = tf.reshape(self.h_conv2,shape=[-1,5,6])
        self.pooled2 = tf.reduce_mean(self.pooled2,reduction_indices=1)
        self.pooled2 = tf.squeeze(self.pooled2)



        self.pooled3 = tf.reshape(self.h_conv3,shape=[-1,4,6])
        ...
        self.pooled3_flatten = tf.reshape(self.pooled3,shape=[-1,5])

        self.flatten = tf.concat([self.pooled2_flatten,self.pooled3_flatten],1)

        num_unit = 300
        H = self.linear(self.pooled2_flatten,self.pooled2_flatten.shape[1],num_unit,'L1')
